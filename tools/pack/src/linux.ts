...
export function renderOpenDesignDebLauncher(defaultNamespace: string): string {
  return [
    ...
    'CLAUDE_CODE_PROFILE="/etc/profile.d/claude-code.sh"',
    ...
    "",
    "load_claude_code_profile() {",
    "  if [ -r \"$CLAUDE_CODE_PROFILE\" ]; then",
    "    set +u",
    "    . \"$CLAUDE_CODE_PROFILE\"",
    "    set -u",
    "  fi",
    ...
    "    printf '[info] ANTHROPIC_BASE_URL: %s\\n' \"$ANTHROPIC_BASE_URL\"",
    "  else",
    "    printf '[info] ANTHROPIC_BASE_URL: not set (using default Anthropic API endpoint)\\n'",
    "  fi",
    "  if [ -n \"${ANTHROPIC_BASE_URL:-}\" ] && command -v curl >/dev/null 2>&1; then",
    "    if curl --connect-timeout 5 --max-time 8 -s -o /dev/null \"$ANTHROPIC_BASE_URL\"; then",
    "      printf '[ok] ANTHROPIC_BASE_URL reachable\\n'",
    "    else",
    "      printf '[warn] ANTHROPIC_BASE_URL did not respond within 5s; verify the proxy/network path\\n'",
    "    fi",
    "  fi",
